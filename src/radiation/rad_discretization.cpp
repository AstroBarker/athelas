...
auto compute_increment_rad_source( const View2D<double> uCR, const int k,
  ...
  for ( int iN = 0; iN < nNodes; ++iN ) {
    ...
    local_sum += grid.get_weights( iN ) * rad_basis->get_phi( iX, iN + 1, k ) *
                 this_source;
  }
...
auto compute_increment_radhydro_source( const View2D<double> uCRH, const int k,
  ...
  for ( int iN = 0; iN < nNodes; ++iN ) {
    ...
    local_sum_e_r += grid.get_weights( iN ) *
                     rad_basis->get_phi( iX, iN + 1, k ) * source_e_r;
    local_sum_m_r += grid.get_weights( iN ) *
                     rad_basis->get_phi( iX, iN + 1, k ) * source_m_r;
    local_sum_e_g += grid.get_weights( iN ) *
                     fluid_basis->get_phi( iX, iN + 1, k ) * source_e_g;
    local_sum_m_g += grid.get_weights( iN ) *
                     fluid_basis->get_phi( iX, iN + 1, k ) * source_m_g;
  }
  // \Delta x / M_kk
