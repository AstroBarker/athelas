...
struct TypeList {
  ...
  static auto ContinuousSublistImpl() {
    return ContinuousSublistImpl<Start>(
        std::make_index_sequence<End - Start + 1>());
  }
...
static auto InsertTypeImpl(TypeList<Ts...>) {
  if constexpr (I == 0) {
    ...
    return ConcatenateTypeLists(
        typename TL::template continuous_sublist<0, I>(), TypeList<T>(),
...
struct ListOfType {
  ...
  using type =
      concatenate_type_lists_t<TypeList<T>, typename ListOfType<Nm1, T>::type>;
};
