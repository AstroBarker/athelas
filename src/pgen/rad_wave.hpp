...
void rad_shock_init( State* state, GridStructure* grid,
  ...
  const Real lambda = pin->in_table["problem"]["params"]["lambda"].value_or( 0.1 );
  const Real kappa = pin->in_table["problem"]["params"]["kappa"].value_or( 1.0 );
  const Real epsilon = pin->in_table["problem"]["params"]["epsilon"].value_or( 1.0e-6 );
  const Real rho0 =
      pin->in_table["problem"]["params"]["rho0"].value_or( 1.0 );
  const Real P0 = pin->in_table["problem"]["params"]["p0"].value_or(
       1.0e-6); // K
