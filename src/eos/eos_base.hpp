...
class EosBase {
 public:
  ...
  auto get_gamma(const double tau, const double V, const double EmT, const double *const lambda) const -> double {
    return static_cast<EOS const*>(this)->get_gamma(tau, V, EmT, lambda);
  }
  [[nodiscard]] auto get_gamma() const -> double { return static_cast<EOS const*>(this)->get_gamma();}
};
