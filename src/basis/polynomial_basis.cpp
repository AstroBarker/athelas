 ...
 **/
void ModalBasis::project_nodal_to_modal(View3D<double> uCF, View3D<double> uPF,
  ...
  for (int k = 0; k < order_; k++) {
    ...
    for (int iN = 0; iN < nNodes_; iN++) {
      ...
      numerator += nodal_val * phi_(iX, iN + 1, k) * 
                   grid->get_weights(iN) * grid->get_widths(iX) * 
                   grid->get_sqrt_gm(X) * rho;
    }
 ...
 * nodal_func: function that takes x coordinate and returns nodal value
 **/
void ModalBasis::project_nodal_to_modal_all_cells(View3D<double> uCF, View3D<double> uPF,
