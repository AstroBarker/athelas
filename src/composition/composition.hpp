...
public:
  ...
  CompositionData(const CompositionKey& comp_mgr, int nX, int nNodes);
  
  // Mass fraction accessors (per element)
  [[nodiscard]] auto mass_fractions() const noexcept -> View3D<double> { return mass_fractions_; }
  
  [[nodiscard]] KOKKOS_INLINE_FUNCTION
  auto element_mass_fraction(int i, int node, int element_idx) const -> double;
  
  // Ionization fraction accessors
  [[nodiscard]] auto ionization_fractions() const noexcept -> View4D<double>;
  
  [[nodiscard]] KOKKOS_INLINE_FUNCTION
  auto ionization_fraction(int i, int element_idx, int node, int charge) const -> double;
  

  // Validate ionization fractions sum to 1 for each element
  KOKKOS_INLINE_FUNCTION
  auto check_ionization_conservation(int i, int node, int element_idx) const -> double {
    double total_f = 0.0;
    const auto& element = comp_mgr_->get_element(element_idx);
    
    for (int charge = 0; charge <= element.max_charge; ++charge) {
  ...
  [[nodiscard]] KOKKOS_INLINE_FUNCTION
  auto sum_mass_fractions(int i, int node) const -> double;
...
private:
  const CompositionKey* comp_mgr_;
  int nX_, nNodes_;
  ...
  [[nodiscard]] auto get_max_states() const noexcept -> int {
    int max_states = 0;
    for (int i = 0; i < comp_mgr_->n_species(); ++i) {
      max_states = std::max(max_states, comp_mgr_->get_element(i).n_states());
    }
    return max_states;
  }
}; // class CompositionData
