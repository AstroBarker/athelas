 ...
 * TODO(astrobarker): Explore hierarchical parallelism for inner loops
 */
void fill_derived_ionization(State *const state, const GridStructure *const grid, const ModalBasis *const basis) {
  static constexpr int ilo = 1;
  static const auto &ihi = grid->get_ihi();
  static const auto &nnodes = grid->get_n_nodes();
  ...
  Kokkos::parallel_for(
      ...
      for (size_t e = 0; e < num_species; ++e) {
            ...
            e_ion_corr(ix, node) += number_density(ix, node) * nk * sum_ion_pot; // e_ion_corr
          }
