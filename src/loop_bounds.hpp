...
// This is a class template that is required for doing something like
// static_assert(false) in constexpr if blocks. Actually writing
// static_assert(false) will always cause a compilation error, even if it is an
// unchosen constexpr if block. This is fixed in C++23 I think.
template <class...>
constexpr std::false_type always_false{};

// struct that can be specialized to register new types that can be processed to
// obtain loop bounds in a par_for* loop
...
constexpr std::size_t GetNumBounds(TypeList<Bnds...>) {
  if constexpr (sizeof...(Bnds) > 0) {
    ...
    using NextBound =
        ProcessLoopBound<base_type<typename TL::template type<0>>>;
...
GetIndexRanges(const int &idx, Kokkos::Array<IndexRange, N> &bound_arr,
  ...
  if constexpr (sizeof...(Bnds) > 0) {
    ...
    ProcessLoopBound<base_type<NextBound>>::GetIndexRanges(
        idx, bound_arr, std::forward<Bnds>(bounds)...);
  }
}
} // namespace LoopBounds
...
struct ProcessLoopBound<Bound, std::enable_if_t<std::is_integral_v<Bound>>>
  ...
  KOKKOS_INLINE_FUNCTION static void
  GetIndexRanges(const int &idx, Kokkos::Array<IndexRange, N> &bound_arr,
                 const int &s, const int &e, Bnds &&...bounds) {
    bound_arr[idx].s = s;
    bound_arr[idx].e = e;
    LoopBounds::GetIndexRanges(idx + 1, bound_arr,
...
// Struct for translating between loop bounds given to par_dispatch into an
// array of IndexRanges
template <class... Bound_ts>
struct LoopBoundTranslator {
 public:
  // make sure all the Bound_ts... types are valid loop bounds and count the
  // number of bounds contained in each type
  static constexpr std::size_t Rank =
      LoopBounds::GetNumBounds(TypeList<Bound_ts...>());
...
constexpr int FirstFuncIdx() {
  if constexpr (idx == TL::n_types) {
    ...
    if constexpr (Functor<cur_type>) {
      return idx;
    }
    if constexpr (std::is_function_v<std::remove_pointer<cur_type>>) {
      return idx;
    }
...
using function_signature =
    FunctionSignature<Rank, decltype(&base_type<F>::operator())>;
} // namespace athelas
