...
auto SmoothnessIndicator( const View3D<Real> U,
...
auto NonLinearWeight( const Real gamma, const Real beta, const Real tau,
                      const Real eps ) -> Real {
  return gamma * ( 1.0 + tau / ( eps + beta ) );
}

// weno-z tau variable
auto Tau( const Real beta_l, const Real beta_i, const Real beta_r,
