...
class SodShockTubeTest(AthelasRegressionTest):
  ...
  def __init__(self, methodName="test_sod", executable_path=None):
    ...
    infile = os.path.join(regression_dir, "test_inputs", "sod.toml")
    goldfile = os.path.join(regression_dir, "goldfiles", "sod.gold")
  ...
  def test_sod(self):
    """Test Sod shock tube simulation against gold standard data."""
    self.run_code()
