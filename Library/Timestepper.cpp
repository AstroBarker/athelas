 ...
 * NOTE: `work` forms inner sum (3D) for timestep and Flux_q(3D) for increment
**/
...
void UpdateFluid( myFuncType ComputeIncrement, double dt, 
  ...
  DataStructure2D& a_jk, DataStructure2D& b_jk,
  std::vector<DataStructure3D> U_s, std::vector<DataStructure3D> dU_s,
  DataStructure3D& dU, DataStructure3D& work, DataStructure2D& dFlux_num, 
  ...
  const short unsigned int nStages, const std::string BC )
{
  ...
  {
    i = iS - 1;
    // re-zero the summation variable `work`
    work.zero();
    ...
    {
      ComputeIncrement( U_s[j], Grid, dU_s[j], work, dFlux_num, uCF_F_L, uCF_F_R, 
                        Flux_U, Flux_P, uCF_L, uCF_R, BC );

      work.zero(); // reset work for summation
      ...
      {
        work(iCF,iX,iN) += a_jk(i,j) * U_s[j](iCF,iX,iN) 
                        + dt * b_jk(i,j) * dU_s[j](iCF,iX,iN);
      }
      
      U_s[i+1] = work; // i = 0.. overwrite initial.. okay?
